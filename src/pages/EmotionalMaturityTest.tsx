/**
 * EmotionalMaturityTest Page
 * Specialized quiz flow for analyzing EQ and emotional responses.
 */
...
const EmotionalMaturityTest: React.FC = () => {
  const navigate = useNavigate();
  const { session, startNewQuiz, selectOption, goNext, goBack, isAnalyzing, finalizeAndAnalyze } = useQuiz();

  useEffect(() => {
    startNewQuiz('emotional', 15);
  }, []);

  if (!session || session.testType !== 'emotional') return <Loader fullScreen />;
  if (isAnalyzing) return <Loader fullScreen title="Evaluating Emotional IQ..." />;
...
const Emoji = styled.div`font-size: 32px; margin-bottom: 8px;`;
...
export default EmotionalMaturityTest;
